...
var table = $('#tableDataView').DataTable({
    ...
    "columns": [
        ...
        {"data": "device"},
        {"data": "device_port"},
        {"data": "is_collect_data"},
        {"data": "description"},
        {"data": "create_time"},
        {"data": "id"},
        {"data": "site_id"},
        {"data": "device_id"},
        {"data": "is_collect_data_id"},
    ],
    "ajax": {
        "url": "/api/sitedevice/get",
        "method": "POST",
        ...
        "dataFilter": function (response) {
            ...
            for (let i = 0; i < responseJson.content.length; i++) {
                ...
                dataRes.data.push({
                    ...
                    "site": responseJson.content[i].site,
                    "device": responseJson.content[i].device,
                    "device_port": responseJson.content[i].device_port,
                    "is_collect_data": responseJson.content[i].is_collect_data == '1' ? "Yes" : "No",
                    ...
                    "device_id": responseJson.content[i].device_id,
                    "is_collect_data_id": responseJson.content[i].is_collect_data,
                })
            }
...
function getData(){
    let data = {
        ...
        "devicePort"	: $('#inputDevicePort').val(),
        "isCollectData"	: $('#inputIsCollectData').val(),
...
function fillDataToForm(rowData) {
    if (rowData != null && rowData != undefined && rowData.length > 0) {
        ...
        $('#inputDeviceInfo').val(rowData[0].device_id);
        $('#inputDevicePort').val(rowData[0].device_port);
        $('#inputIsCollectData').val(rowData[0].is_collect_data_id);
...
$('#tableDataView thead tr:eq(1) th').each( function (i) {
    ...
    if (title != '#' && !colBypassInputSearch.includes(i)) {
        if (i == 1) {
            ...
            $(this).html( "<select class='selectpicker form-control' >"+ $('#inputDeviceInfo').html() + '</select>');
            $( 'select', this ).on( 'keyup change', function () {
                if ( table.column(i).search() !== this.value ) {
                    table
                        .column(i)
